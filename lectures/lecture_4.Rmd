...
# Revision of the Labs

Everything in **R** is an *object* 

You’ve already worked with several key types of objects:
...
Each object has **functions** you can apply to it. These are typically written with parentheses `()`:
...
We have seen `<-` can be used to create or update objects. 
```r
x <- 5
name <- "Sebastian"
data <- read.csv("file.csv")
```
You can check what type of object something is using the `class()` function
```r
x <- 5
class(x)
#> [1] "numeric"
...
We have seen `$` can be used to access information from dataframes.
```r
data$income
employee$name
```
...
This week we will construct our own functions. 

This will enable us to automate repetitive tasks and speed up our work.
...
#> "Hello, Sebastian!"
```
Try to make this function work on your own before the lab.
...
It is a measure of certain types of economic activity, not quality of life. 
