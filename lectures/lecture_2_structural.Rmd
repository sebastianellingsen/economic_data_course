...
\begin{equation}
\ln X_{od}^k = \ln \lambda_k + \ln A_o^k - \theta_k\ln r_o -\theta_k \hat{\delta} \text{LCRED}\left(\mathbf{R}_t, \mathbf{\hat{a}}\right)_{odt} + \theta_k p_d^k + \ln X_d^k
\end{equation}
...
- $\text{RAIN}_{ot}^k$ is the rainfall in location $o$ at time $t$ in growing season for $k$
...
## Class of Spatial Models Used for Regions
