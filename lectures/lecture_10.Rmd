...
The coursework will be uploaded on Friday. 

It should be handed in by the following Friday. 
...
We have seen some important principlies for visualization:

- The *data to ink ratio*.

- The *lie factor*.

- Tables and figures need to be reproducible to be useful. 
...
- Also a statistical discrapency.
...
Sum of all *production flows*: output approach (or output approach)
...
Just like we eliminated price differences across time, with a the consumer price index.
...
We explored the role of seasonality adjustment in monthly tie series data.
...
We saw how to work on data on inequality and how to calcualte the Gini index.
...
1. More complext data structures.
...
Another widely used unconventional datasourse used in economics is text. 

- Any information that is stored and written in a text format. 

- It can be anything from emails to blog posts to social media posts, online forum comments etc.s
...
Working on these kinds of datasources requires programming knowledge. 

- The data sources are unstructured, extensive work to get them into a useful format.

- More computational resources avaiable to work on these data in the last decade. 

Novel statistical models to understand the properties of these data. 

- Often very high dimensional.

- Need tools to reduce the dimension, eg. machine learning. 
